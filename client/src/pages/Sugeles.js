...
const Surgeles = () => {
  ...
  const handleClick = () => {
    ...
    return (
      <>
      <h1 className="text-yellow-400 text-center w-screen p-1">Surgelés</h1>

      <div className='flex flex-col items-center bg-white w-screen h-screen p-4'>
        <div>
          <h1 className='text-center'>Surgelés prices review</h1>
          <div className='text-center bg-black text-white w-60 rounded-full m-5'>
            <button onClick={handleClick}>Scrape Surgelés</button>
          </div> 
        </div>
      <div>
